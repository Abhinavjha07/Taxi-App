...
@database_sync_to_async
def create_trip(
    pick_up_address = '119 Gali No. 5 Shakti Nagar',
    drop_off_address = 'Bara Hazari',
    status = 'REQUESTED',
    rider = None,
    driver = None ):
...
class TestWebSocket:
    ...
    async def test_request_trip(self, settings):
        ...
        assert response_data['pick_up_address'] == '129 Gali No.-5'
        assert response_data['drop_off_address'] == 'Bara Hazari'
    ...
    async def test_create_trip_group(self, settings):
        ...
        await communicator.send_json_to({
            ...
            'data': {
                'pick_up_address': '119 Gali No. 5 Shakti Nagar',
                'drop_off_address': 'Bara Hazari Road',
                'rider': user.id,
            },
        })
